...
import {faBriefcase, faPen, faTrash, faPlusCircle} from "@fortawesome/free-solid-svg-icons"
import {firebaseClient} from "../../firebase/firebaseClient"
import 'firebase/firestore';
import AddEducation from "./inputs/add-employment"
import EditEducation from "./inputs/edit-employment"
...
const Employment = (props) => {
    ...
    function Delete(id){
        let db = firebaseClient.firestore();
        db.collection('users').doc(props.user.uid).collection('employment').doc(id).delete()
        .then(res => {
            console.log(res)
            console.log("success")
            FetchData();
    ...
    function ListItem(props) {
        ...
        return (
            <li className="profile-row">
                <h3 className="profile-row-heading">{props.value.job_title}</h3>
                <div className="container">
                    <div className="profile-column">
                        <h4 className="profile-row-text"> {props.value.company} - {props.value.location}</h4>
                        <span className="profile-row-text"> {props.value.start_date} to {props.value.currently_employed === true ? "Present" : props.value.end_date}</span>
                    </div>
                    {edit ?
                ...
                <div className="wt-container profile-container">
                    <div className="profile-icon-container">
                        <div className="profile-section-icon" >
                            <FontAwesomeIcon className="fa-3x centre-translate profile-section-icon" icon={faBriefcase}/>
                        </div>
                    </div>
